...
for motif = motifs2process;
    ...
    legend_cell{f_no==f} = [num2str(all_freqs(f)) 'Hz'];
        ...
        i = i +1; 
        results = find(counter.motif(motif).freq(f).dBandDC);
        number.motif(motif).freq(f).dBandDC = length(results);
        
        if number.motif(motif).freq(f).dBandDC > 0;
            value(:,i) = [number.motif(motif).freq(f).dBandDC];
            h.bar(f) = bar(value/Total_sims, 'FaceColor', eval(Colors{f}));
            xlabel('Motif Number', 'FontSize', 48, 'FontWeight', 'bold', 'Color', 'r');
            title('Sustained Responders', 'FontSize', 48, 'FontWeight', 'bold', 'Color', 'r'); 
            ylabel('Occurrences per 10,000 Events','FontSize', 48, 'FontWeight', 'bold', 'Color', 'r');
    ...
    ax.FontSize = 12;
    ylim([0 5*10^-3]);
    set(gca, 'YTickLabel', [0:5:50]);
    

        
end
dim = [.2 .5 .3 .3];
str = { 'Sustained Response > 0.1','Decibel  > 20'};
