...
ind_r = [3];%,77046,84601,91203,91366,95849,98847]; %Which index would you like to plot?


all_freqs =  [0.001 0.003 0.01 0.03 0.1 0.3 1];
freqs =  {'001' ,'003','01', '03', '1/10', '3/10', '1'};
f_no = [1:7]; %Select more than one frequency
compare = true; %select to compare motif at all freqs at specific index value

home_dir_input = 'D:/JAMIL/Jamil/Oscillators_HighRes_Test_input/output_dir';
home_dir = 'D:/JAMIL/Jamil/Oscillators_HighRes_Test/output_dir';
if compare == true;
   i = 0;
    for f = f_no
        t = [-100:-1 0:0.1:2000]; %[-1000:-1 0:1/(20*all_freqs(f)):20/all_freqs(f)];%%%%%%%Time Vector
        t1 = [-2000:0.1:0]; 
        input = .4*sin(2*pi*all_freqs(f)*t)+.5; %Input Oscillatory Wave
        for motif = motifs2process
            output_in = importdata([home_dir_input '/output_motif_osc_' num2str(motif) '/' num2str(all_freqs(f)) 'Hz.txt'])';
            ...
            for ind = ind_r
                ...
                subplot(4,1,1)
                in = 0.5*ones(1,length(t1));
                plot(t1(t1<=0), in,'r');
                hold on;
                plot(t(t>=0),input(:,101:20101), 'r'); %Plots input
                title(['Input - ' num2str(all_freqs(f)) ' Hz']); xlabel('Time (secs)'); ylabel('Amplitude');
                hold off; 
                subplot(4,1,2)
                plot(t1(t1<=0),output_in(ind,101:20101)); %Plots output 
                hold on;
                plot(t(t>=0),output(ind,101:20101),'b');
                title('Output'); xlabel('Time (secs)'); ylabel('Amplitude');
                hold off;
                fs = 10; %20*all_freqs(f); %Sampling Frequency
                y = output_in(ind,101:20101); %output signal
               ...
               [pks_out, index] = findpeaks(y); % Finds peaks of output for t>0
                ...
                hold off;
                y = output(ind,101:20101); %output signal
